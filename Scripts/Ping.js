...
function CPing(pingFunction, delay, start)
{
    ...
    this.iLongDelay = Math.max(delay, 2000);
    this.iShortDelay = 1000;
    this.handlePingResult = null;
    this.handlePingFailure = null;
    ...
    {
        var timeout = this.iLongDelay;
        if (error) timeout = this.iShortDelay;
        window.setTimeout("oPing.Ping();", timeout);
    }
    ...
    this.PingSuccess = function (result, context, method) {
        ...
        if (minutes < 10)
            minutes = "0" + minutes
        //window.status = 'Ping Complete at:' + hours + ':' + minutes;
